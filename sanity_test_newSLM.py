...
def test_inference(c_star, e_tilde, k_tilde, col):
    ...
    data = json.dumps({
        "model": "Qwen3.5-0.8B.Q4K-uav-flight",  # <-- Your new model name!
        "prompt": raw_prompt,
...
print("Test 1: Fragmentation (Expect target_algo: 2)")
test_inference(c_star=0.04, e_tilde=0.01, k_tilde=0.9, col=0)
print("Test 2: Collision (Expect not_sure)")
test_inference(c_star=0.95, e_tilde=0.01, k_tilde=0.1, col=1)
print("Test 3: Nominal (Expect adjust_squad_gains)")
